...
def create_figure(plot_config: plotruns_lib.FigureProperties):
    ...
    matplotlib.rc('legend', fontsize=int(plot_config.default_legend_fontsize * font_scale))
    
    # Additional settings for Agg backend compatibility
    matplotlib.rc('font', size=int(plot_config.axes_fontsize * font_scale))  # Base font size
...
def plot_run_to_png(plot_config: plotruns_lib.FigureProperties,
                   outfile: str,
                   time_indices: Optional[List[int]] = None,
                   output_dir: str = "png_plots") -> List[str]:
    """
...
def plot_run_to_gif(plot_config: plotruns_lib.FigureProperties,
                   outfile: str,
    ...
    if actual_frames == len(available_indices):
        ...
        selected_indices = np.linspace(0, len(available_indices)-1, actual_frames, dtype=int)
        time_indices = [available_indices[i] for i in selected_indices]

    print(f"🎬 Creating animated GIF with {actual_frames} frames (frame_skip={frame_skip})...")
    ...
    for frame_idx, time_idx in enumerate(time_indices):
        ...
        lines1 = _get_lines_at_time(plot_config, plotdata1, axes, time_idx)
        if plotdata2:
            lines2 = _get_lines_at_time(plot_config, plotdata2, axes, time_idx, comp_plot=True)
...
def _get_lines_at_time(plot_config: plotruns_lib.FigureProperties,
                      plotdata1: plotruns_lib.PlotData,
                      axes: List[Any],
                      time_idx: int,
                      comp_plot: bool = False) -> List[Any]:
    """
