...
def torax_app_fixture():
    with patch('gymtorax.torax_wrapper.torax_app.build_runtime_params'), \
         ...
         patch('gymtorax.torax_wrapper.torax_app.run_loop.run_loop', return_value=([MagicMock()], [MagicMock()], torax_app.state.SimError.NO_ERROR)), \
...
def test_run_returns_false_on_sim_error():
    ...
    with patch('gymtorax.torax_wrapper.torax_app.build_runtime_params'), \
         patch('gymtorax.torax_wrapper.torax_app.initial_state_lib.get_initial_state_and_post_processed_outputs', return_value=(MagicMock(), MagicMock())), \
         patch('gymtorax.torax_wrapper.torax_app.output.StateHistory') as MockStateHistory, \
         patch('gymtorax.torax_wrapper.torax_app.run_loop.run_loop', return_value=([MagicMock()], [MagicMock()], 999)), \
         patch('gymtorax.torax_wrapper.torax_app.DataTree'):
        instance = MockStateHistory.return_value
        instance.simulation_output_to_xr.return_value = MagicMock()
...
def test_reset_with_restart_true():
    ...
    with patch('gymtorax.torax_wrapper.torax_app.build_runtime_params'), \
         patch('gymtorax.torax_wrapper.torax_app.initial_state_lib.get_initial_state_and_post_processed_outputs', return_value=(MagicMock(), MagicMock())), \
         patch('gymtorax.torax_wrapper.torax_app.initial_state_lib.get_initial_state_and_post_processed_outputs_from_file', return_value=(MagicMock(), MagicMock())), \
         patch('gymtorax.torax_wrapper.torax_app.output.StateHistory') as MockStateHistory, \
         patch('gymtorax.torax_wrapper.torax_app.run_loop.run_loop', return_value=([MagicMock()], [MagicMock()], torax_app.state.SimError.NO_ERROR)), \
         patch('gymtorax.torax_wrapper.torax_app.DataTree'):
        instance = MockStateHistory.return_value
